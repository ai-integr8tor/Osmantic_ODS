...
| M4 | Voice agent defaulted to unencrypted `ws://` | Superseded / accepted local risk | The current [`ods/extensions/services/dashboard/src/pages/ODSTalk.jsx`](ods/extensions/services/dashboard/src/pages/ODSTalk.jsx) client uses same-origin HTTP and server-sent event requests, so HTTPS pages keep the transport on HTTPS; live microphone access is also gated on a secure browser context. [`ods/SECURITY.md`](ods/SECURITY.md) recommends TLS or VPN for network exposure. | Plain HTTP on localhost/LAN remains cleartext by design; use TLS or Tailscale/WireGuard for sensitive shared deployments. |
